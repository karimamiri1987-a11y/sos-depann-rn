...
export default function TDFLeaderboardScreen({ navigation }) {
  ...
  const totalPossible = completedStages * (5 + 4 + 3 + 1);

  // Build per-participant point breakdown
  const buildBreakdown = (participantId) => {
    const myRiders = draw[participantId] || [];
    let pts5 = 0, pts4 = 0, pts3 = 0, pts1 = 0;
    const winStages = [];

    Object.entries(stageResults).forEach(([stageId, result]) => {
      const stage = TDF_STAGES.find(s => s.id === stageId);
      if (!result) return;
      if (result.p1 && myRiders.includes(result.p1)) { pts5++; winStages.push({ stageNum: stage?.num, place: 1, riderId: result.p1 }); }
      if (result.p2 && myRiders.includes(result.p2)) { pts3++; winStages.push({ stageNum: stage?.num, place: 2, riderId: result.p2 }); }
      if (result.p3 && myRiders.includes(result.p3)) { pts1++; winStages.push({ stageNum: stage?.num, place: 3, riderId: result.p3 }); }
      if (result.combatif && myRiders.includes(result.combatif)) { pts4++; winStages.push({ stageNum: stage?.num, place: 'C', riderId: result.combatif }); }
    });
    return { pts5, pts4, pts3, pts1, winStages };
  };
              ...
              return (
                <View key={participant.id} style={styles.rankCard}>
                  ...
                  <View style={styles.breakdown}>
                    ...
                    <BreakItem icon="🔥" count={breakdown.pts4} pts={4} />
                        ...
                        return (
                          <View key={i} style={styles.winChip}>
                            <Text style={styles.winChipText}>
                              Ét.{w.stageNum} {w.place === 'C' ? '🔥' : ['🥇', '🥈', '🥉'][w.place - 1]} {rider?.name.split(' ').pop()}
                            </Text>
                          </View>
                        );
                      })}
...
const styles = StyleSheet.create({
  ...
  breakdown: { flexDirection: 'row', flexWrap: 'wrap', gap: 12, marginBottom: 8 },
