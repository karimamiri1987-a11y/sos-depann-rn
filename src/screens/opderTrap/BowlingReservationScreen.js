...
function getNextDays(n = 14) {
  ...
  return Array.from({ length: n }, (_, i) => {
    ...
    return { id: i, short: d.getDate().toString(), day: JOURS[d.getDay()],
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.infoBanner}>
            <Text style={styles.infoText}>
              💡 Tarif : {BOWLING.tarifHeure}€/heure/personne · Chaussures incluses
            </Text>
          </View>
          ...
          {/* Joueurs */}
          <View style={styles.card}>
            <Label icon="people-outline" text="Joueurs et pistes" />
            <View style={styles.stepperSection}>
              <Text style={styles.stepperLabel}>Joueurs (max 6 par piste)</Text>
              <Stepper value={players} min={2} max={12} onChange={setPlayers} color={CYAN} />
            </View>
            <View style={[styles.stepperSection, { borderTopWidth: 1, borderTopColor: ODT.border, paddingTop: 12 }]}>
              <Text style={styles.stepperLabel}>Nombre de pistes</Text>
              <Stepper value={lanes} min={1} max={2} onChange={setLanes} color={CYAN} />
            </View>
          </View>
